resource "aws_subnet" "group-two-db-subnet" {
  vpc_id = "${var.vpc_id}"
...
resource "aws_route_table_association" "group-two-db-rta" {
  subnet_id = "${aws_subnet.group-two-db-subnet.id}"
  route_table_id = "${var.route_table_id}"
}

resource "aws_security_group" "group-two-sg-db-group" {
  name = "${var.name}-db-sg"
...
resource "aws_security_group_rule" "rule" {
  ...
  security_group_id = "${aws_security_group.group-two-sg-db-group.id}"
}
